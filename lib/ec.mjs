...
export function tally(fills, { includeSelfTrades = false } = {}) {
  ...
  const group = (map, key, volume, quantity, maker, taker) => {
    let g = map.get(key)
    if (!g) {
      g = { key, volume: 0n, contracts: 0n, fills: 0, traders: new Set(), makers: new Set() }
      map.set(key, g)
    }
    g.volume += volume
    g.contracts += quantity
    g.fills++
    // A trader is the taker side. Every EC market is quoted by a fresh
    // market-maker wallet, so counting makers would count markets, not people.
    g.traders.add(taker)
    g.makers.add(maker)
  }
  ...
  const flatten = (map) =>
    [...map.values()]
      .map(({ traders, makers, ...g }) => ({ ...g, traders: traders.size, makers: makers.size }))
      .sort(byVolume)

  // Same rule for the headline: traders are addresses that took at least one
  // fill; maker-only addresses are the venue's per-market quoting wallets.
  let traders = 0
  let makerWallets = 0
  for (const r of rows.values()) {
    if (r.takerFills > 0) traders++
    if (r.makerFills > 0) makerWallets++
  }

  return {
    list: [...rows.values()].sort(byVolume),
    stats: { ...stats, traders, makerWallets, addresses: rows.size },
